...
description: 在飞书群中同步 WindClaw 的 `.windclaw-aigw-session` 到另一个 OpenClaw。技能只创建当前端需要的一个 agent，并根据本机是否存在 `.windclaw-aigw-session` 自动判断当前端是发送端还是接收端。
---
