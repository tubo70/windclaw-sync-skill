...
这个技能只能在 WindClaw 环境初始化，也就是与 `openclaw.json` 同级目录存在 `.windclaw-aigw-session` 时才允许使用。非 WindClaw 端不安装、不初始化这个技能；目标 OpenClaw 只需要理解群消息并把 sessionId 覆盖写入 `~/.openclaw-windclaw/.windclaw-aigw-session`。